...
export default function Products() {
  ...
  const [categoryName, setCategoryName] = useState(state?.categoryName || 'Products');
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const res = await fetch(`${BACKEND_URL}/api/catalog/category/${categoryId}`);
      ...
      <main className="flex-1 transition-all duration-300 pt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="mb-8">
            ...
            <p className="text-foreground/60 mt-2">{filteredProducts.length} items available</p>
          </div>
